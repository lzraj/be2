...
////////////////////CREATE////////////////////
    ...
    app.post("/ideas", (req, res) => {
        const sql = `
        INSERT INTO ideas (title, idea, goal, image, user_id)
        VALUES (?, ?, ?, ?, ?)
        `;
        con.query(sql, [req.body.title, req.body.idea, req.body.goal, req.body.image, req.body.user_id], (err, result) => {
            if (err) throw err;
            res.send({ msg: 'OK', text: 'New idea was added.', type: 'success' });
        });
    });
...
app.get("/home/ideas", (req, res) => {
    const sql = `
    SELECT i.*, g.id as gid, g.sum, g.name, g.idea_id FROM ideas AS i LEFT JOIN givers AS g ON g.idea_id = i.id ORDER BY i.title
    `;
    con.query(sql, (err, result) => {
        if (err) throw err;
        res.send(result);
